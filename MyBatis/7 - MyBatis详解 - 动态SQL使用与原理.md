...
- MyBatis详解 - 动态SQL使用与原理
  - 动态SQL官方使用参考
    ...
    - [bind](#bind)
    - [多数据库支持](#多数据库支持)
    - [动态 SQL 中的插入脚本语言](#动态-sql-中的插入脚本语言)
  - 动态SQL解析原理
    - [关于动态SQL的接口和类](#关于动态sql的接口和类)
    - [源码分析走起](#源码分析走起)
    - [实例分析](#实例分析)

## [#](#动态sql官方使用参考) 动态SQL官方使用参考
...
![img](./../笔记图片/mybatis-y-dynamic-sql-1.png)
...
![img](./../笔记图片/mybatis-y-dynamic-sql-2.png)

BoundSql类，封装mybatis最终产生sql的类，包括sql语句，参数，参数源数据等参数：

![img](./../笔记图片/mybatis-y-dynamic-sql-4.png)

XNode，一个Dom API中的Node接口的扩展类：

![img](./../笔记图片/mybatis-y-dynamic-sql-5.png)

BaseBuilder接口及其实现类(属性，方法省略了，大家有兴趣的自己看),这些Builder的作用就是用于构造sql：

![img](./../笔记图片/mybatis-y-dynamic-sql-6.png)
...
![img](./../笔记图片/mybatis-y-dynamic-sql-7.png)
...
![img](./../笔记图片/mybatis-y-dynamic-sql-8.png)

由于XMLConfigBuilder内部也是使用XMLMapperBuilder，我们就看看XMLMapperBuilder的解析细节：

![img](./../笔记图片/mybatis-y-dynamic-sql-9.png)

![img](./../笔记图片/mybatis-y-dynamic-sql-10.png)

我们关注一下，增删改查节点的解析：

![img](./../笔记图片/mybatis-y-dynamic-sql-11.png)

XMLStatementBuilder的解析：

![img](./../笔记图片/mybatis-y-dynamic-sql-12.png)

默认会使用XMLLanguageDriver创建SqlSource（Configuration构造函数中设置）。

XMLLanguageDriver创建SqlSource：

![img](./../笔记图片/mybatis-y-dynamic-sql-13.png)

XMLScriptBuilder解析sql：

![img](./../笔记图片/mybatis-y-dynamic-sql-14.png)
...
![img](./../笔记图片/mybatis-y-dynamic-sql-15.png)

trim节点：

![img](./../笔记图片/mybatis-y-dynamic-sql-16.png)

由于这个update方法是个动态节点，因此构造出了DynamicSqlSource。DynamicSqlSource内部就可以构造sql了:

![img](./../笔记图片/mybatis-y-dynamic-sql-17.png)
...
著作权归@pdai所有 原文链接：https://pdai.tech/md/framework/orm-mybatis/mybatis-y-dynamic-sql.html