...
对ReentrantReadWriteLock的操作基本上都转化为了对Sync对象的操作，只需分析Sync即可。
...
![img](./../笔记图片/java-thread-x-readwritelock-1.png)
...
![img](./../笔记图片/java-thread-x-readwritelock-2.png)
...
protected final boolean tryAcquire(int acquires) {
    ...
    if (c != 0) { // 状态不为0
        ...
        if (w == 0 || current != getExclusiveOwnerThread()) // 写线程数量为0或者当前线程为独占锁线程
            return false;
...
![img](./../笔记图片/java-thread-x-readwritelock-3.png)
...
![img](./../笔记图片/java-thread-x-readwritelock-4.png)
...
![img](./../笔记图片/java-thread-x-readwritelock-5.png)
